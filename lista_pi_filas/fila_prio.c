...
int isemptyfp(TFilaPrio *fp, int prio) {
    switch(prio){
        case 1: return isempty(&fp->f1);
        case 2: return isempty(&fp->f2);
        case 3: return isempty(&fp->f3);
    }
    return 1;
}
...
void storefp(TFilaPrio *fp, int prio, ITEM x) {
    switch(prio){
        case 1: store(&fp->f1, x); break;
        case 2: store(&fp->f2, x); break;
        case 3: store(&fp->f3, x); break;
    }
}

ITEM retrievefp(TFilaPrio *fp, int prio) {
    switch(prio){
        case 1: return retrieve(&fp->f1);
        case 2: return retrieve(&fp->f2);
        case 3: return retrieve(&fp->f3);
    }
    return 0;
}
